...
exports.createTask = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!title || pointsValue === undefined || !assignedTo || assignedTo.length === 0) {
        throw new applicationError_1.default('Missing required fields: title, pointsValue, and at least one assignedTo ID are required.', 400);
    }
...
exports.getTaskById = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!task) {
        throw new applicationError_1.default('No task found with that ID in this household.', 404);
    }
    res.status(200).json({
        status: 'success',
        data: {
...
exports.completeTask = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (memberId) {
        ...
        if (!memberProfile) {
            throw new applicationError_1.default('Member not found in this household.', 404);
        }
    }
    else {
        ...
        if (!memberProfile) {
            throw new applicationError_1.default('Your member profile was not found in this household.', 404);
        }
    }
    // 3. Find the task
    const task = await Task_1.default.findOne({ _id: taskId, householdId: householdId });
    if (!task) {
        throw new applicationError_1.default('Task not found.', 404);
    }
    ...
    if (!isAssigned) {
        throw new applicationError_1.default('This member is not assigned to this task.', 403);
    }
...
exports.approveTask = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!task) {
        throw new applicationError_1.default('Task not found or is not pending approval.', 404);
    }
    if (!task.completedBy) {
        throw new applicationError_1.default('Task cannot be approved: completedBy field is missing.', 400);
    }
    ...
    if (!memberProfile) {
        throw new applicationError_1.default('Member profile who completed task not found.', 404);
    }
