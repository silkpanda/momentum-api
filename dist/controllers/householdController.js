...
exports.createHousehold = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!creatorFamilyMemberId) {
        throw new applicationError_1.default('Authentication error. User not found.', 401);
    }
    if (!householdName || !userDisplayName || !userProfileColor) {
        throw new applicationError_1.default('Missing required fields: householdName, userDisplayName, and userProfileColor are all required.', 400);
    }
...
exports.getMyHouseholds = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!householdId) {
        throw new applicationError_1.default('Household context not found in session token.', 401);
    }
    ...
    if (!household) {
        throw new applicationError_1.default('Primary household not found.', 404);
    }
    res.status(200).json({
        status: 'success',
        data: household,
...
exports.getHousehold = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!household) {
        throw new applicationError_1.default('Household not found.', 404);
    }
    ...
    if (!isMember) {
        throw new applicationError_1.default('You are not a member of this household.', 403);
    }
    res.status(200).json({
        status: 'success',
        data: household,
...
exports.updateHousehold = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!householdName) {
        throw new applicationError_1.default('householdName is required for update.', 400);
    }
    ...
    if (!memberProfile || memberProfile.role !== 'Parent') {
        throw new applicationError_1.default('Unauthorized. Only Parents can update household details.', 403);
    }
...
exports.deleteHousehold = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const memberProfile = household.memberProfiles.find((p) => p.familyMemberId.toString() === userId.toString());
    if (!memberProfile || memberProfile.role !== 'Parent') {
        throw new applicationError_1.default('Unauthorized. Only Parents can delete a household.', 403);
    }
...
exports.addMemberToHousehold = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!displayName || !profileColor || !role) {
        if (!familyMemberId && (!firstName || !role)) {
            throw new applicationError_1.default('Missing required fields: displayName, profileColor, and role are required. For new members, firstName is also required.', 400);
        }
    }
    if (!familyMemberId) {
        if (role !== 'Child') {
            throw new applicationError_1.default('Only the "Child" role can be created through this endpoint without a familyMemberId.', 400);
        }
    ...
    const isParent = household.memberProfiles.some((member) => member.familyMemberId.equals(loggedInUserId) &&
        member.role === 'Parent');
    if (!isParent) {
        throw new applicationError_1.default('Unauthorized. Only parents of this household can add new members.', 403);
    }
    const isAlreadyMember = household.memberProfiles.some((member) => member.familyMemberId.equals(familyMemberId));
    if (isAlreadyMember) {
        throw new applicationError_1.default('This family member is already in the household.', 400);
    }
    const memberExists = await FamilyMember_1.default.findById(familyMemberId);
    if (!memberExists) {
        throw new applicationError_1.default('No family member found with the provided ID.', 404);
    }
...
exports.updateMemberProfile = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const isParent = household.memberProfiles.some((member) => member.familyMemberId.equals(loggedInUserId) &&
        member.role === 'Parent');
    if (!isParent) {
        throw new applicationError_1.default('Unauthorized. Only parents of this household can update members.', 403);
    }
    const memberProfile = household.memberProfiles.find((member) => member._id.equals(memberProfileId));
    if (!memberProfile) {
        throw new applicationError_1.default('Member profile not found in this household.', 404);
    }
    if (displayName)
...
exports.removeMemberFromHousehold = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const isParent = household.memberProfiles.some((member) => member.familyMemberId.equals(loggedInUserId) &&
        member.role === 'Parent');
    if (!isParent) {
        throw new applicationError_1.default('Unauthorized. Only parents of this household can remove members.', 403);
    }
    ...
    if (memberToRemove.role === 'Parent') {
        ...
        if (parentCount <= 1) {
            throw new applicationError_1.default('Cannot remove the last parent from a household.', 400);
        }
    }
...
exports.getInviteCode = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!isParent)
        throw new applicationError_1.default('Unauthorized', 403);
...
exports.joinHousehold = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const user = req.user;
    if (!inviteCode)
        throw new applicationError_1.default('Invite code is required', 400);
    const household = await Household_1.default.findOne({ inviteCode: inviteCode.toUpperCase() });
    if (!household)
        throw new applicationError_1.default('Invalid invite code', 404);
    const isMember = household.memberProfiles.some((p) => p.familyMemberId.toString() === userId.toString());
    if (isMember)
        throw new applicationError_1.default('You are already a member of this household', 400);
    const newProfile = {
