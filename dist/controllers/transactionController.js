...
const purchaseStoreItem = async (req, res) => {
    try {
        ...
        if (!household) {
            ...
            const hhExists = await Household_1.default.findById(householdId);
        ...
        const memberProfile = household?.memberProfiles.find((p) => p.familyMemberId.toString() === memberId.toString() // Robust comparison
        );
        if (!memberProfile) {
